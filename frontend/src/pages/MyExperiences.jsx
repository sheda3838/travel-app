import React, { useState, useEffect, useCallback } from 'react';
import api from '../api/axios';
import ExperienceCard from '../components/ExperienceCard';
import Pagination from '../components/Pagination';

function MyExperiences() {
  const [experiences, setExperiences] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  
  // Pagination state
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);

  const fetchMyExperiences = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/experiences/mine?page=${page}&limit=6`);
      setExperiences(data.data);
      setTotalPages(data.pages);
      setPage(data.page);
      setError(null);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to fetch your experiences');
    } finally {
      setLoading(false);
    }
  }, [page]);

  useEffect(() => {
    fetchMyExperiences();
  }, [fetchMyExperiences]);

  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this experience? This action cannot be undone.")) {
      try {
        await api.delete(`/experiences/${id}`);
        // Remove locally from state rather than refetching or just refetch
        fetchMyExperiences(); 
      } catch (err) {
        console.error(err);
        alert(err.response?.data?.message || "Failed to delete experience");
      }
    }
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">My Experiences</h1>
        <p className="text-gray-600 mt-2">Manage the experiences you've shared with the community.</p>
      </div>

      {loading ? (
        <div className="text-center py-20 text-xl font-medium text-gray-500">
          Loading your experiences...
        </div>
      ) : error ? (
        <div className="text-center py-20 text-red-500">
          {error}
        </div>
      ) : experiences.length === 0 ? (
        <div className="text-center py-20 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-300">
          <p className="text-xl mb-4">You haven't posted any experiences yet.</p>
          <a href="/create" className="text-blue-600 font-medium hover:underline">
            Create your first experience here
          </a>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {experiences.map((exp) => (
              <ExperienceCard 
                key={exp._id} 
                experience={exp} 
                onDelete={handleDelete} 
              />
            ))}
          </div>
          
          <Pagination page={page} pages={totalPages} onPageChange={setPage} />
        </>
      )}
    </div>
  );
}

export default MyExperiences;