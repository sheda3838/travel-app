...
function Home() {
  ...
  const fetchExperiences = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/experiences?page=${page}&limit=6&q=${searchQuery}`);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <h1 className="text-3xl font-bold text-gray-800">Discover Experiences</h1>
        <form onSubmit={handleSearch} className="flex w-full md:w-auto">
          ...
          />
          <button type="submit" className="bg-blue-600 text-white px-5 py-2 rounded-r-md hover:bg-blue-700 transition">
            Search
          </button>
        </form>
      </div>
